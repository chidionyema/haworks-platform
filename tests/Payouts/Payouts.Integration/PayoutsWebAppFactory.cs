...
public class PayoutsWebAppFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
 ...
 { builder.UseEnvironment("Test"); builder.ConfigureAppConfiguration((_, config) => { config.AddInMemoryCollection(new Dictionary<string, string?> { ["ConnectionStrings:payouts"] = ConnString, ["Stripe:SecretKey"] = "sk_test_dummy" }); }); builder.ConfigureTestServices(services => { var mockGateway = new Mock<IPayoutGateway>(); mockGateway.Setup(x => x.InitiatePayoutAsync(It.IsAny<string>(), It.IsAny<decimal>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync(("tr_test", Haworks.Payouts.Domain.Enums.PayoutStatus.Succeeded)); services.AddSingleton(mockGateway.Object); services.AddMassTransitTestHarness(); services.AddAuthentication(TestAuthenticationHandler.SchemeName).AddTestAuth(); }); }
}
