...
public sealed class LedgerServiceTests : IDisposable
{
    ...
    public async Task CreditSellerAsync_creates_three_entries()
    {
        ...
        await _service.CreditSellerAsync(sellerId, 10000L, "USD", Guid.NewGuid(), "Test payment");
    ...
    public async Task CreditSellerAsync_is_idempotent_on_same_referenceId()
    {
        ...
        await _service.CreditSellerAsync(sellerId, 10000L, "USD", refId, "Payment 1");
        await _context.SaveChangesAsync(); // outbox handles this in production
        await _service.CreditSellerAsync(sellerId, 10000L, "USD", refId, "Payment 1 duplicate");
        await _context.SaveChangesAsync();
    ...
    public async Task DebitSellerAsync_reverses_credit()
    {
        ...
        await _service.CreditSellerAsync(sellerId, 10000L, "USD", paymentId, "Payment");
        await _context.SaveChangesAsync();

        await _service.DebitSellerAsync(sellerId, 10000L, "USD", paymentId, "Refund");
        await _context.SaveChangesAsync();
    ...
    public async Task DebitSellerAsync_is_idempotent()
    {
        ...
        await _service.DebitSellerAsync(sellerId, 10000L, "USD", paymentId, "Refund");
        await _context.SaveChangesAsync();
        await _service.DebitSellerAsync(sellerId, 10000L, "USD", paymentId, "Refund duplicate");
        await _context.SaveChangesAsync();
