...
public class StockReservationTimeoutWatcherTests
{
    ...
    private readonly Mock<ILogger<StockReservationTimeoutWatcher>> _mockLogger;
    private readonly Mock<DbSet<CheckoutSagaState>> _mockDbSet;
    private readonly StockReservationTimeoutWatcher _watcher;

    public StockReservationTimeoutWatcherTests()
    {
        ...
        _mockLogger = new Mock<ILogger<StockReservationTimeoutWatcher>>();
        ...
        _watcher = new StockReservationTimeoutWatcher(_mockScopeFactory.Object, _mockLogger.Object);
    }

    [Fact]
    public async Task TickAsync_WithStuckInitiatedSagas_ShouldPublishTimeoutEvents()
    {
        // Arrange
        ...
        var stuckSagas = new List<CheckoutSagaState>
        {
            new()
            {
                ...
                CreatedAt = now.AddMinutes(-10) // Past 5-minute timeout
            },
            new()
            {
                CorrelationId = Guid.NewGuid(),
                OrderId = Guid.NewGuid(),
                CurrentState = "Initiated",
                CreatedAt = now.AddMinutes(-8) // Past timeout
            }
        };
        ...
        _mockPublishEndpoint.Verify(x => x.Publish(
            It.Is<StockReservationTimedOutEvent>(e => e.SagaId == stuckSagas[0].CorrelationId && e.OrderId == stuckSagas[0].OrderId),
            It.IsAny<CancellationToken>()), Times.Once);

        _mockPublishEndpoint.Verify(x => x.Publish(
            It.Is<StockReservationTimedOutEvent>(e => e.SagaId == stuckSagas[1].CorrelationId && e.OrderId == stuckSagas[1].OrderId),
            It.IsAny<CancellationToken>()), Times.Once);

        VerifyLogInformation("StockReservationTimeoutWatcher: publishing timeout for 2 stuck saga(s)");
    }
    ...
    public async Task TickAsync_WithNoStuckSagas_ShouldNotPublishAnyEvents()
    {
        ...
        // Assert
        _mockPublishEndpoint.Verify(x => x.Publish(It.IsAny<StockReservationTimedOutEvent>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    [Fact]
    public async Task TickAsync_WithRecentInitiatedSagas_ShouldNotPublishEvents()
    {
        // Arrange
        var now = DateTime.UtcNow;
        var recentSagas = new List<CheckoutSagaState>
        {
            new()
            {
                ...
                OrderId = Guid.NewGuid(),
                CurrentState = "Initiated",
                CreatedAt = now.AddMinutes(-2) // Within 5-minute timeout
            }
        };
    ...
    [Fact]
    public async Task TickAsync_WithNonInitiatedStates_ShouldNotPublishEvents()
    {
        // Arrange
        var now = DateTime.UtcNow;
        var nonInitiatedSagas = new List<CheckoutSagaState>
        {
            new()
            {
                CorrelationId = Guid.NewGuid(),
                OrderId = Guid.NewGuid(),
                CurrentState = "StockReservedState", // Not Initiated
            ...
            new()
            {
                CorrelationId = Guid.NewGuid(),
                OrderId = Guid.NewGuid(),
                CurrentState = "Completed", // Not Initiated
                CreatedAt = now.AddMinutes(-15)
            }
        };

        var queryable = nonInitiatedSagas.AsQueryable();
        SetupDbSetQueryable(queryable, new List<object>());
    ...
    public async Task TickAsync_WithPublishFailure_ShouldLogWarningAndContinue()
    {
        ...
        _mockPublishEndpoint.Setup(x => x.Publish(It.IsAny<StockReservationTimedOutEvent>(), It.IsAny<CancellationToken>()))
        ...
        // Assert
        VerifyLogWarning($"StockReservationTimeoutWatcher: failed to publish for saga {stuckSaga.CorrelationId}; will retry next tick");
    }

    [Fact]
    public async Task TickAsync_WithMaxBatchSize_ShouldLimitPublishCount()
    {
        // Arrange
        var now = DateTime.UtcNow;
        ...
        // Assert
        _mockPublishEndpoint.Verify(x => x.Publish(It.IsAny<StockReservationTimedOutEvent>(), It.IsAny<CancellationToken>()), Times.Exactly(50));
        VerifyLogInformation("StockReservationTimeoutWatcher: publishing timeout for 50 stuck saga(s)");
    }
    ...
    public async Task TickAsync_ShouldOrderByCreatedAtAndProcessOldestFirst()
    {
        ...
        // Assert
        _mockPublishEndpoint.Verify(x => x.Publish(It.IsAny<StockReservationTimedOutEvent>(), It.IsAny<CancellationToken>()), Times.Exactly(2));

        // Both should be published, verifying the oldest one exists
        _mockPublishEndpoint.Verify(x => x.Publish(
            It.Is<StockReservationTimedOutEvent>(e => e.SagaId == oldestSaga.CorrelationId),
            It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    private async Task InvokeTickAsync()
    {
        ...
        var method = typeof(StockReservationTimeoutWatcher).GetMethod("TickAsync", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
