...
public sealed class OrderTests
{
    private static IEnumerable<(Guid productId, string productName, int quantity, long unitPriceCents)>
        OneItem() => new[] { (Guid.NewGuid(), "Widget", 1, 1_000L) };

    private static Order NewOrder(long totalCents = 1_000L, string userId = "user-1") =>
    ...
    {
        Action act = () => Order.Create("", 1_000L, "USD", Guid.NewGuid(), "k", "e@x.com", OneItem());
        act.Should().Throw<ArgumentException>();
    }
    ...
    {
        Action act = () => Order.Create("u", 1_000L, "USD", Guid.Empty, "k", "e@x.com", OneItem());
        act.Should().Throw<ArgumentException>();
    }
...
{
    [Fact]
    public void Create_computes_LineTotalCents()
    {
        var item = OrderItem.Create(Guid.NewGuid(), Guid.NewGuid(), "Widget", 3, 1_050L);
        item.LineTotalCents.Should().Be(3_150L);
    }

    [Fact]
    public void Create_with_zero_quantity_throws()
    {
        Action act = () => OrderItem.Create(Guid.NewGuid(), Guid.NewGuid(), "Widget", 0, 100L);
        act.Should().Throw<ArgumentException>();
    }

    [Fact]
    public void Create_with_negative_unitPriceCents_throws()
    {
    ...
    {
        Action act = () => OrderItem.Create(Guid.NewGuid(), Guid.Empty, "Widget", 1, 100L);
        act.Should().Throw<ArgumentException>();
    }
}
