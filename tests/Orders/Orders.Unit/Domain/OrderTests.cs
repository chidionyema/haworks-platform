...
public class OrderTests
{
    ...
    public void Create_WithInvalidUserId_ThrowsArgumentException(string? userId)
    {
        Assert.ThrowsAny<ArgumentException>(() =>
            Order.Create(userId!, 10_000L, "USD", Guid.NewGuid(), "key", "email", DefaultItems));
    }
    ...
    {
        Assert.Throws<ArgumentException>(() =>
            Order.Create("user", 10_000L, "USD", Guid.Empty, "key", "email", DefaultItems));
    }
    ...
    public void Create_WithEmptyItems_ThrowsArgumentException()
    {
        ...
        Assert.Throws<ArgumentException>(() =>
            Order.Create("user", 10_000L, "USD", Guid.NewGuid(), "key", "email", emptyItems));
    }
    ...
    {
        // Arrange
        var order = Order.Create("user", 10_000L, "USD", Guid.NewGuid(), "key", "email", DefaultItems);
    ...
    public void Create_ItemsAreCorrectlyPopulated()
    {
        ...
        {
            (Guid.NewGuid(), "Product A", 2, 5_000L),
            (Guid.NewGuid(), "Product B", 1, 3_000L)
        };

        // Act
        var order = Order.Create("user", 13_000L, "USD", Guid.NewGuid(), "key", "email", items);

        // Assert
        order.Items.Should().HaveCount(2);
        order.Items.Should().Contain(i => i.ProductName == "Product A" && i.Quantity == 2 && i.UnitPriceCents == 5_000L);
        order.Items.Should().Contain(i => i.ProductName == "Product B" && i.Quantity == 1 && i.UnitPriceCents == 3_000L);
    }
