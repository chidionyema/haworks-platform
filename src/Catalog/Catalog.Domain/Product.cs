...
public class Product : AuditableEntity
{
    ...
    private Product(string name, string description, long unitPriceCents, string currency, Guid categoryId)
        : base()
    {
        Name = name;
    ...
    public static Product Create(string name, string description, long unitPriceCents, string currency, Guid categoryId)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(name);
        ArgumentException.ThrowIfNullOrWhiteSpace(currency);
        if (unitPriceCents < 0) throw new ArgumentException("Price cannot be negative", nameof(unitPriceCents));
        return new Product(name, description, unitPriceCents, currency, categoryId);
    }
    ...
    public void UpdatePricing(long unitPriceCents, string currency)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(currency);
        if (unitPriceCents < 0) throw new ArgumentException("Price cannot be negative", nameof(unitPriceCents));
        UnitPriceCents = unitPriceCents;
