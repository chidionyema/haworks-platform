...
/// ritualworks-{{feature}} flycast endpoint via the shared
...
public class {{FEATURE}}SmokeTests(EnvironmentAgnosticFixture fixture)
{
    ...
    {
        var resp = await fixture.HttpClient.GetAsync("http://ritualworks-{{feature}}.flycast:8080/health");

        resp.StatusCode.Should().NotBe(System.Net.HttpStatusCode.NotFound,
            "ritualworks-{{feature}} must be deployed and serving /health");
